...
items = data.entries.filter(x => x.category == cat);
...
function addItem(){
  ...
  window.location = "/#/edit/entries/"+newItem.id;
...
{#each items as item}
  ...
  {:else}
  <a href="/#/edit/entries/{item.id}">{#if item.title==''}Untitled{:else}{item.title}{/if}</a>
  {/if}
  </div>
  <div class="col-6 text-right">
  {#if cat !== 'collections'}
